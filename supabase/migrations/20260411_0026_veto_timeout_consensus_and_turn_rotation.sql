-- 20260411_0026_veto_timeout_consensus_and_turn_rotation.sql
-- Rules enforced:
-- 1) No map is removed at click-time, only when turn timer expires.
-- 2) If same-team votes are split across multiple maps (top tie), remove nothing and rotate turn.
-- 3) If top map vote count is below required threshold, remove nothing and rotate turn.
...
begin
  ...
  if p_veto_map is null then
    ...
    -- No valid consensus by timeout:
    -- - no votes
    -- - top votes below threshold
    -- - tie at top (e.g. split votes on different maps)
