...
alter table public.quick_queue_ready_checks
  add column if not exists stake_amount numeric(14,2) not null default 0;

update public.quick_queue_entries
set selected_stake_amount = coalesce(selected_stake_amount, 5)
where selected_stake_amount is null;
