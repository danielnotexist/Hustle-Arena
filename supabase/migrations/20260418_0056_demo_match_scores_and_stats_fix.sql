...
begin
  ...
  loop
    ...
    perform public.create_notification(
      ...
      case
        when v_is_winner then format('Victory %s-%s · You won %.2f USDT', v_score_for, v_score_against, greatest(v_user_delta, 0))
        else format('Defeat %s-%s · You lost %.2f USDT', v_score_for, v_score_against, abs(least(v_user_delta, 0)))
      end,
      '/battlefield',
      jsonb_build_object(
        'match_id', p_match_id,
